...
import { ISolidityCompiler, IVyperCompiler, SolidityJsonInput, SourcifyChain, VyperJsonInput, VyperCompilation, SolidityCompilation, Sources, } from "@ethereum-sourcify/lib-sourcify";
...
export const getContractPathFromSourcesOrThrow = (
  ...
  console.debug("etherscan-util: Parsing for all sources done", {
    contractName,
    contractPath,
...
export const fetchFromEtherscan = async (
  ...
  console.debug("Fetching from Etherscan", {
    url,
  ...
  console.debug("Fetched from Etherscan", {
    url,
    chainId: sourcifyChain.chainId,
    address,
  });
  ...
  if (
    ...
    console.info("Etherscan Rate Limit", {
      url,
      chainId: sourcifyChain.chainId,
      address,
      resultJson,
  ...
  if (resultJson.result[0].SourceCode === "") {
    console.info("Contract not found on Etherscan", {
      url,
      chainId: sourcifyChain.chainId,
      address,
    });
...
export const processSolidityResultFromEtherscan = (
  ...
  if (isEtherscanJsonInput(sourceCodeObject)) {
    console.debug("Etherscan solcJsonInput contract found");
    ...
    console.debug("Etherscan Solidity multiple file contract found");
    ...
    console.debug("Etherscan Solidity single file contract found");
...
export const processVyperResultFromEtherscan = async (
  ...
  if (isEtherscanJsonInput(sourceCodeProperty)) {
    console.debug("Etherscan vyperJsonInput contract found");
    ...
    console.debug("Etherscan Vyper single file contract found");
